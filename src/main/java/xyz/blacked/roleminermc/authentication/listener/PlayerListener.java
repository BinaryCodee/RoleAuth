package xyz.blacked.roleminermc.authentication.listener;
...
import xyz.blacked.roleminermc.authentication.tasks.CrackedTask;
import xyz.blacked.roleminermc.authentication.tasks.PremiumTask;
        ...
        if (timeoutSeconds > 0) {
            plugin.getProxy().getScheduler().schedule(plugin, () -> {
                if (player.isConnected() && plugin.getUtils().isAuthenticating(uuid)) {
                    player.disconnect(plugin.getUtils().colorize(plugin.getUtils().getMessage("authentication-timeout")));
                }
